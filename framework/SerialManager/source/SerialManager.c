                                     ...
                                     uint8_t instance )
{
    ...
    *pInterfaceId = gSerialMgrInvalidIdx_c;
    ...
    for ( i=0; i<gSerialManagerMaxInterfaces_c; i++ )
    {
        ...
        {
            OSA_InterruptDisable();
            pSer->serialChannel = i;
            pSer1->serialChannel = i+1;
            ...
            case gSerialMgrLpuart_c:
                ...
                (void)LPUART_Initialize(i, &mDrvData[i].uartState);
                mDrvData[i].uartState.pRxData = pSer->rxBuffer;
                mDrvData[i].uartState.rxSize = 1;
                (void)LPUART_InstallRxCalback(i, Serial_UartRxCb, i);
                (void)LPUART_InstallTxCalback(i, Serial_UartTxCb, i);

                (void)LPUART_Initialize(i+1, &mDrvData[i+1].uartState);
                mDrvData[i+1].uartState.pRxData = pSer1->rxBuffer;
                mDrvData[i+1].uartState.rxSize = 1;
                (void)LPUART_InstallRxCalback(1, Serial_UartRxCb, i+1);
                (void)LPUART_InstallTxCalback(1, Serial_UartTxCb, i+1);
#endif
                break;
            ...
            if( status == gSerial_Success_c )
            {
                ...
                pSer1->serialType = interfaceType;
